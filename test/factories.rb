...
#:experiment_modelling and :experiment_experimental rely on the existence of the ExperimentClass's
...
#Protocol
  Factory.define(:protocol) do |f|
    f.title "This Protocol"
...
#Experiment and Technology types

  Factory.define(:technology_type, :class=>TechnologyType) do |f|
    f.sequence(:title) {|n| "A TechnologyType#{n}"}
  end

  Factory.define(:experiment_type) do |f|
    f.sequence(:title) {|n| "An ExperimentType#{n}"}
  end

#Experiment
Factory.define(:experiment_base, :class => Experiment) do |f|
  f.title "An Experiment"
    f.association :contributor, :factory => :person
  f.association :study
  f.association :experiment_type

end

Factory.define(:modelling_experiment_class, :class => ExperimentClass) do |f|
  f.title 'Modelling Experiment'
  f.key 'MODEL'
end

Factory.define(:experimental_experiment_class, :class => ExperimentClass) do |f|
  f.title 'Experimental Experiment'
  f.key 'EXP'
end

Factory.define(:modelling_experiment, :parent => :experiment_base) do |f|
  f.association :experiment_class, :factory => :modelling_experiment_class
end

Factory.define(:modelling_experiment_with_organism, :parent => :modelling_experiment) do |f|
  f.after_create{|ma|Factory.build(:organism,:experiment=>ma)}

end
Factory.define(:experimental_experiment, :parent => :experiment_base) do |f|
  f.association :experiment_class, :factory => :experimental_experiment_class
  f.association :technology_type
  f.samples {[Factory.build :sample]}
end

  Factory.define(:experiment, :parent => :modelling_experiment) {}
...
#either url or data should be provided for assets
  ...
  Factory.define(:experimental_condition) do |f|
    ...
    f.association :measured_item, :factory => :measured_item
    f.association :unit, :factory => :unit
    f.association :protocol, :factory => :protocol
