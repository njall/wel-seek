...
class PolicyBasedAuthTest < ActiveSupport::TestCase
  ...
  test "has advanced permissions" do
    ...
    experiment = Factory :experimental_experiment,:policy=>Factory(:all_sysmo_viewable_policy),:contributor=>user.person,:study=>Factory(:study, :investigation=>Factory(:investigation,:projects=>[proj1,proj2]))
    experiment.policy.permissions << Factory(:permission,:contributor=>proj1,:access_type=>Policy::EDITING)
    experiment.policy.permissions << Factory(:permission,:contributor=>proj2,:access_type=>Policy::EDITING)
    experiment.save!
    assert !experiment.has_advanced_permissions?
    proj_permission = Factory(:permission,:contributor=>Factory(:project),:access_type=>Policy::EDITING)
    experiment.policy.permissions << proj_permission
    assert experiment.has_advanced_permissions?
    experiment.policy.permissions.delete(proj_permission)
    experiment.save!
    assert !experiment.has_advanced_permissions?
    experiment.policy.permissions << Factory(:permission,:contributor=>Factory(:project),:access_type=>Policy::VISIBLE)
    assert experiment.has_advanced_permissions?
  end
