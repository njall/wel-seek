...
class SendPeriodicEmailsJobTest < ActiveSupport::TestCase
  ...
  test "perform" do
    ...
    protocol = Factory(:protocol, :policy => Factory(:public_policy))
    project_subscription1 = ProjectSubscription.create(:person_id => person1.id, :project_id => protocol.projects.first.id, :frequency => 'daily')
    project_subscription2 = ProjectSubscription.create(:person_id => person2.id, :project_id => protocol.projects.first.id, :frequency => 'weekly')
    project_subscription3 = ProjectSubscription.create(:person_id => person3.id, :project_id => protocol.projects.first.id, :frequency => 'monthly')
    project_subscription4 = ProjectSubscription.create(:person_id => person4.id, :project_id => protocol.projects.first.id, :frequency => 'monthly')
    ...
    Factory :activity_log,:activity_loggable => protocol, :culprit => Factory(:user), :action => 'create'
  ...
  test "perform ignores unwanted actions" do
    ...
    assert_emails 0 do
      ...
      Factory :activity_log,:activity_loggable => protocol, :culprit => Factory(:user), :action => 'show'
      Factory :activity_log,:activity_loggable => protocol, :culprit => Factory(:user), :action => 'download'
      Factory :activity_log,:activity_loggable => protocol, :culprit => Factory(:user), :action => 'destroy'
  ...
  test "perform2" do
    ...
    protocol = Factory(:protocol, :policy=>Factory(:private_policy),:contributor=>person1,:projects=>[project1])
    ...
    disable_authorization_checks do
      Factory :activity_log, :activity_loggable=>protocol, :culprit=>Factory(:user),:action=>"update"
