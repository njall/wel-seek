...
class DataFileTest < ActiveSupport::TestCase
  ...
  test "experiment association" do
    User.with_current_user Factory(:user) do
      ...
      assert_not_equal experiment_asset.asset, datafile
      assert_not_equal experiment_asset.experiment, experiment
      experiment_asset.asset = datafile
      experiment_asset.experiment = experiment
      experiment_asset.relationship_type = relationship
      experiment_asset.save!
      experiment_asset.reload

      assert experiment_asset.valid?
      assert_equal experiment_asset.asset, datafile
      assert_equal experiment_asset.experiment, experiment
      assert_equal experiment_asset.relationship_type, relationship
    end
  end
  ...
  test "managers" do
    ...
    manager=people(:person_for_owner_of_my_first_protocol)
  ...
  test "can get relationship type" do
    ...
    experiment = experiments(:modelling_experiment_with_data_and_relationship)
    assert_equal relationship_types(:validation_data), df.relationship_type(experiment)
  end
