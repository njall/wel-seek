...
class UserTest < ActiveSupport::TestCase
  ...
  fixtures :users,:protocols,:data_files,:models,:assets
  ...
  def test_get_assets
    ...
    assert user.protocols.size>0
    assert user.protocols.include?(protocols(:my_first_protocol))
    assert !user.protocols.include?(protocols(:protocol_with_fully_public_policy))
