...
class SendImmediateEmailsJobTest < ActiveSupport::TestCase
  ...
  test "perform" do
    ...
    project_subscription1 = ProjectSubscription.create(:person_id => person1.id, :project_id => protocol.projects.first.id, :frequency => 'immediately')
    project_subscription2 = ProjectSubscription.create(:person_id => person2.id, :project_id => protocol.projects.first.id, :frequency => 'immediately')
    ...
    assert_emails 2 do
      disable_authorization_checks do
        al = ActivityLog.create(:activity_loggable => protocol, :culprit => Factory(:user), :action => 'create')
