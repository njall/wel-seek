...
class PoliciesControllerTest < ActionController::TestCase
  ...
  test 'should show the preview permission when choosing blacklist and whitelist' do
    ...
    post :preview_permissions, :sharing_scope => 0, :use_blacklist => 'true', :use_whitelist => 'true', :resource_name => 'protocol'
  ...
  test 'can publish experiment without study' do
    a_person = Factory(:person)
    experiment = Experiment.new

    login_as(a_person.user)
    assert experiment.can_manage?
    assert experiment.can_publish?

    updated_can_publish = PoliciesController.new().updated_can_publish('experiment', experiment.id, '')
    assert updated_can_publish
  end

  test 'can not publish experiment having project with gatekeeper' do
    gatekeeper = Factory(:gatekeeper)
    a_person = Factory(:person)
    experiment = Experiment.new
    experiment.study = Factory(:study, :investigation => Factory(:investigation, :projects => gatekeeper.projects))

    login_as(a_person.user)
    assert experiment.can_manage?
    assert !experiment.can_publish?

    updated_can_publish = PoliciesController.new().updated_can_publish('experiment', experiment.id, experiment.study.id.to_s)
    assert !updated_can_publish
  end
  ...
  test 'should show the preview permission for resource without projects' do
    ...
    post :preview_permissions, :sharing_scope => 2, :access_type => Policy::VISIBLE, :project_access_type => Policy::ACCESSIBLE, :project_ids => '0', :resource_name => 'experiment'
    assert_response :success

    post :preview_permissions, :sharing_scope => 2, :access_type => Policy::VISIBLE, :project_access_type => Policy::ACCESSIBLE, :project_ids => '0', :resource_name => 'protocol'
    assert_response :success
  end
end