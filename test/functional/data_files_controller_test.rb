...
class DataFilesControllerTest < ActionController::TestCase
  ...
  test "associates experiment" do
    ...
    original_experiment = experiments(:metabolomics_experiment)
    asset_ids = original_experiment.related_asset_ids 'DataFile'
    assert asset_ids.include? d.id

    new_experiment=experiments(:metabolomics_experiment2)
    new_asset_ids = new_experiment.related_asset_ids 'DataFile'
    assert !new_asset_ids.include?(d.id)
    assert_difference('ActivityLog.count') do
      put :update, :id => d, :data_file =>{}, :experiment_ids=>[new_experiment.id.to_s]
    end
    ...
    assert !original_experiment.related_asset_ids('DataFile').include?(d.id)
    assert new_experiment.related_asset_ids('DataFile').include?(d.id)
  end
  ...
  test "should create data file" do
    login_as(:datafile_owner) #can edit experiment
    experiment=experiments(:experiment_can_edit_by_datafile_owner)

    assert_difference('ActivityLog.count') do
      assert_difference('DataFile.count') do
        assert_difference('ContentBlob.count') do
          post :create, :data_file => valid_data_file, :sharing=>valid_sharing, :experiment_ids => [experiment.id.to_s]
        end
      end
    end
    assert_redirected_to data_file_path(assigns(:data_file))
    ...
    assert assigns(:data_file).content_blob.url.blank?
    experiment.reload
    assert experiment.related_asset_ids('DataFile').include? assigns(:data_file).id
  end
