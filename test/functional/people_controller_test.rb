...
class PeopleControllerTest < ActionController::TestCase
  ...
  test 'unsubscribe to a project should unsubscribe all the items of that project' do
    ...
    protocol = Factory(:protocol, :projects => [proj], :policy => Factory(:public_policy))
    ...
    assert protocol.subscribed?(current_person)
    assert df.subscribed?(current_person)
    assert current_person.receive_notifications?

    assert_emails 1 do
      Factory(:activity_log, :activity_loggable => protocol, :action => 'update')
