...
class FavouriteGroupsControllerTest < ActionController::TestCase
  ...
  test "add duplicate fails" do
    login_as(:owner_of_a_protocol_with_complex_permissions)
    fav=favourite_groups(:my_collaborators_group_for_owner_of_a_protocol_with_complex_permissions)
    user=fav.user

    #check that the user is the expected one for the test to be valid
    assert_equal users(:owner_of_a_protocol_with_complex_permissions),user,"Not the expected user, so the rest of the test is invalid"
  ...
  test "update duplicate fails" do
    login_as(:owner_of_a_protocol_with_complex_permissions)
    fav=favourite_groups(:my_collaborators_group_for_owner_of_a_protocol_with_complex_permissions)
    other_fav=favourite_groups(:whitelist_for_owner_of_a_protocol_with_complex_permissions)
    user=fav.user
