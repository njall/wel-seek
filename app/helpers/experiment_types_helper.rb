module ExperimentTypesHelper
  ...
  #edit and remove icons, and the number of experiments associated with the node.
    ...
    unless parent.children.empty?
      parent.children.sort{|a,b| a.title.downcase <=> b.title.downcase}.each do |child|
        result << "<li style=\"margin-left:#{12*depth}px;#{child.id == selected_id ? "background-color: lightblue;" : ""}\">"+ (depth>0 ? "└ " : " ") + (link_to child.title, child) + " " +
                    ...
                    (show_delete ? (child.experiments.size == 0 ? link_to(image("destroy"),child, :confirm =>
                      "Are you sure you want to remove this #{child.class.name}?  This cannot be undone.",
                      :method => :delete, :style=>"vertical-align:middle") : "<span style=\"color: #666666;\">(#{child.experiments.size} experiments)</span>") : "") +
                    "</li>"
