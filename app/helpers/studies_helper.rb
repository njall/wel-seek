module StudiesHelper
  ...
  def new_experiment_to_investigation_popup_link investigation_id
    return link_to_remote_redbox("New experiment",
      {:url=>new_experiment_url(:investigation_id=>investigation_id),
        :failure => "alert('Sorry, an error has occurred.'); RedBox.close();" },
      {:id => "create_new_experiment_redbox"}
    )
  end
