...
#  end
  ...
  map.resources :experiment_types, :collection=>{:manage=>:get}
  ...
  map.resources :experiments,:member=>{:update_annotations_ajax=>:post}
  ...
  map.resources :protocols, :member => { :download => :get, :new_version=>:post, :preview_publish=>:get,:publish=>:post,:request_resource=>:post, :update_annotations_ajax=>:post } do |protocol|
    protocol.resources :experimental_conditions, :collection =>{:create_from_existing=>:post}
  end
