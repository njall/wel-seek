    ...
    module PolicyBasedAuthorization
      ...
      module AuthLookupClassMethods
        ...
        def all_authorized_for action, user=User.current_user, projects=nil
          ...
          assets = []
          programatic_project_filter = !projects.nil? && (!Seek::Config.auth_lookup_enabled || (self==Experiment || self==Study))
        ...
        def lookup_for_action_and_user action,user_id,projects
          #Study's and Experiments have to be treated differently, as they are linked to a project through the investigation'
          if (projects.nil? || (self == Study || self == Experiment))
      ...
      def contributing_user
        unless self.kind_of?(Experiment)
