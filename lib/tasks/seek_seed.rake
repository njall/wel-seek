...
namespace :seek do
  ...
  task :refresh_controlled_vocabs=>[:environment,:culture_growth_types, :model_types, :model_formats, :experiment_types, :disciplines, :organisms, :technology_types, :recommended_model_environments, :measured_items, :units, :project_roles, :experiment_classes, :relationship_types]
  ...
  task(:experiment_types=>:environment) do
    ...
    Fixtures.create_fixtures(File.join(RAILS_ROOT, "config/default_data"), "experiment_types")
  end
  ...
  task(:experiment_classes=>:environment) do
    revert_fixtures_identify
    ExperimentClass.delete_all
    Fixtures.create_fixtures(File.join(RAILS_ROOT, "config/default_data"), "experiment_classes")
  end
